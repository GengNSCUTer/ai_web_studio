...
- 后续新增或修改的代码注释、docstring、测试用例中的解释性注释，默认使用中文；只有第三方协议字段、标准术语或必须与外部英文接口保持一致的内容保留英文。
- 每次交付说明除描述功能和验证结果外，还应列出主要实现文件及其职责，帮助用户将设计结论对应到具体代码位置。
- 每次交付还必须用简短链路说明“代码如何实现该功能”：列出入口、关键校验/状态转换和输出边界；不得只复述设计文档或把计划中的能力表述为已经运行的代码。
- 历史代码中的英文注释不要求为了语言统一进行纯机械翻译；只有在本次功能改动触及该段逻辑时，再随改动一并中文化，避免制造无意义 diff。
